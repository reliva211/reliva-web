...
export async function searchBooks(query: string): Promise<BookSearchResult[]> {
  try {
    // Searching books with query

    // OpenLibrary search API
    const response = await fetch(
      `https://openlibrary.org/search.json?q=${encodeURIComponent(
        query
      )}&limit=20&fields=key,title,author_name,first_publish_year,cover_i,edition_count,has_fulltext,ebook_access,ebook_count,edition_key,first_sentence,subject,isbn,language,number_of_pages_median,publisher,first_sentence,excerpts`
    );
    ...
    // OpenLibrary search response received
...
export async function getBookDetails(
  ...
  try {
    ...
    const data = await response.json();
    // OpenLibrary book details response received
    ...
    const excerpt =
      data.excerpts && data.excerpts.length > 0 ? data.excerpts[0].excerpt : "";

    const firstSentence =
      data.first_sentence && data.first_sentence.length > 0
    ...
    return {
      ...
      cover: coverUrl,
      overview:
        description || excerpt || firstSentence || "No description available.",
      ...
      languages: data.languages
        ? data.languages.map((lang: any) => lang.key)
        : [],
