...
import { Search, Film, Star, Plus, Check, X, Calendar, ListFilter, Eye, EyeOff } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { setDoc, doc, deleteDoc, updateDoc, collection, getDocs } from "firebase/firestore"
...
export default function MoviesPage() {
  ...
  const [savedIds, setSavedIds] = useState(savedMovies.map((movie) => movie.id))
  ...
  const saveMovie = async (movie: Movie) => {
    ...
    if (!savedIds.includes(movie.id)) {
      const newMovie = { ...movie, status: "Watchlist", rating: 0, notes: "" }
  ...
  const filteredMovies =
    filterStatus === "all" ? savedMovies : savedMovies.filter((movie) => movie.status === filterStatus)
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 space-y-6">
          ...
          <div className="space-y-2">
            <h3 className="font-medium">My Movies</h3>
            <div className="space-y-1">
              ...
              <button
                className={`w-full flex items-center justify-between rounded-md p-2 text-sm ${filterStatus === "Watched" ? "bg-accent" : "hover:bg-accent transition-colors"}`}
                onClick={() =>     {setSearchQuery(""); setIsSearching(false); setFilterStatus("Watched");}}
              >
                <span>Watched</span>
              ...
              <button
                className={`w-full flex items-center justify-between rounded-md p-2 text-sm ${filterStatus === "Watchlist" ? "bg-accent" : "hover:bg-accent transition-colors"}`}
                onClick={() => {setSearchQuery(""); setIsSearching(false);setFilterStatus("Watchlist");}}
              >
                <span>Watchlist</span>
                <span className="text-xs bg-muted px-2 py-0.5 rounded-full">
                  {savedMovies.filter((movie) => movie.status === "Watchlist").length}
                </span>
              </button>
            </div>
          </div>
              ...
              <TabsContent value="my-movies" className="space-y-6">
                ...
                {filteredMovies.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium mb-2">Your movie collection is empty</h3>
                    <p className="text-muted-foreground mb-4">Search for movies to add to your collection</p>
                  ...
                  <ScrollArea className="h-[calc(100vh-16rem)]">
                    <div className="space-y-4">
                      {filteredMovies.map((movie) => (
                        <div key={movie.id} className="flex border rounded-lg overflow-hidden">
                          ...
                          <div className="flex-1 p-4 flex flex-col justify-between">
                            <div>
                              ...
                              {movie.status === "Watched" && (movie.rating ?? 0) > 0 && (
                                <div className="flex items-center mt-1">
                                  {Array.from({ length: 5 }).map((_, i) => (
                                    <Star
                                      key={i}
                                      className={`h-4 w-4 ${i < (movie.rating ?? 0) ? "fill-primary text-primary" : "text-muted"}`}
                                    />
                                  ))}
                                </div>
                              )}
                              {movie.notes && <p className="text-sm mt-2 line-clamp-2">{movie.notes}</p>}
                            </div>
                            <div className="mt-2 flex gap-2">
                              <Button
                                variant={movie.status === "Watchlist" ? "default" : "outline"}
                                size="sm"
                              ...
                              <Dialog>
                                <DialogTrigger asChild>
                                  <Button variant="outline" size="sm" onClick={() => openMovieDetails(movie)}>
                                    Add Notes
                                  </Button>
                                </DialogTrigger>
                                <DialogContent>
                                  ...
                                  <div className="space-y-4 py-4">
                                      ...
                                      <div className="flex gap-1">
                                        {Array.from({ length: 5 }).map((_, i) => (
                                          <Button
                                            ...
                                            size="icon"
                                            className="h-8 w-8"
                                            onClick={() => setMovieRating(i + 1)}
                                          >
                                            <Star
                                              className={`h-6 w-6 ${i < movieRating ? "fill-primary text-primary" : "text-muted-foreground"}`}
                                            />
                                          </Button>
                                        ))}
                                      </div>
                                    ...
                                    <div className="flex justify-end gap-2">
                                      ...
                                      <Button onClick={saveMovieDetails}>Save</Button>
                                    </div>
                                  </div>
                                </DialogContent>
                              </Dialog>
