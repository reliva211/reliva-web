...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetch(
      `https://api.musicapi.com/spotify/search?q=${encodeURIComponent(
        query
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `MusicAPI search failed: ${response.status} - ${errorText}`
      );
    }
...
export async function POST(request: Request) {
  try {
    const { accessToken, action, playlistId, trackId } = await request.json();
    ...
    switch (action) {
      case "getUserPlaylists":
        endpoint = `https://api.musicapi.com/spotify/users/${accessToken}/playlists`;
        break;
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `MusicAPI operation failed: ${response.status} - ${errorText}`
      );
    }
